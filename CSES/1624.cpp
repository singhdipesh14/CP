...
void search(int r, int *ways, bool col[], bool diag1[], bool diag2[], bool reserved[][8])
{
	if (r == 8)
	{
		(*ways)++;
	...
	for (int c = 0; c < 8; c++)
	{
		...
		search(r + 1, ways, col, diag1, diag2, reserved);
...
void func()
{
	bool col[16] = {false}, diag1[16] = {false}, diag2[16] = {false}, reserved[8][8];
	char c;
	for (int i = 0; i < 8; i++)
	{
		for (int j = 0; j < 8; j++)
	...
	int ways = 0;
	search(0, &ways, col, diag1, diag2, reserved);
	cout << ways << '\n';
}
